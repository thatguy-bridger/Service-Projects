...
# one Feature per line) down to just Draper, UT -- run this locally where
...
# and adjust the CITY value below if it doesn't say exactly "DRAPER".
set -euo pipefail

INPUT="${1:?Usage: ./filter-draper.sh /path/to/statewide.geojson}"
CITY="DRAPER"
OUTPUT="$(dirname "$INPUT")/draper.geojson"
