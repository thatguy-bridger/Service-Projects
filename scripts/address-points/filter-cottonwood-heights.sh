...
# one Feature per line) down to just Cottonwood Heights, UT -- run this
# locally where the big file actually lives; nothing here talks to the
# network or the app. Requires jq (brew install jq / apt install jq).
#
...
# and adjust the CITY value below if it doesn't say exactly
# "COTTONWOOD HEIGHTS".
set -euo pipefail

INPUT="${1:?Usage: ./filter-cottonwood-heights.sh /path/to/statewide.geojson}"
CITY="COTTONWOOD HEIGHTS"
OUTPUT="$(dirname "$INPUT")/cottonwood-heights.geojson"
